...
const generateLink = (token, option) => {
  return `http://localhost:${PORT}/auth/volunteer/${option}?token=${token}`;
};
...
class authController {
  async whoAmI(req, res) {
    try {
      const user = await Volunteer.findOne({ _id: req.volunteer._id }, '-password -__v')
      if (!user) {
  ...
  async register(req, res) {
    try {
      ...
      const candidate = await Volunteer.findOne({ email });
      ...
      if (candidate && !candidate.verified) {
        await Volunteer.deleteOne({ _id: candidate._id });
      }
  ...
  async verify(req, res) {
    try {
      ...
      let user = await Volunteer.findOne({ emailToken: token });
      if (!user) {
        res.status(400).json({ error: "Internal error" });
      }
  ...
  async sendReset(req, res) {
    try {
      ...
      let candidate = await Volunteer.findOne({ email });
      if (!candidate) {
        res.status(400).json({ error: "Internal error" });
      }
      const token = generateVerifToken(email)
  ...
  async resetPassword(req, res) {
    try {
      ...
      //check is link valid
      let user = await Volunteer.findOne({ resetToken: token });
      if (!user) {
        res.status(400).json({ error: "Internal error" });
      }
